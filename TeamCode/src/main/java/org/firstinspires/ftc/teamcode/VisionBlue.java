...
public class VisionBlue extends OpenCvPipeline {
            ...
            public Mat processFrame(Mat input) {
                ...
                if (rightmean[0]> bluescalarlow.val[0] && rightmean[0]<bluescalarhigh.val[0]){
                    if (rightmean[1] > bluescalarlow.val[1]&& rightmean[1] < bluescalarhigh.val[1]){
                        if (rightmean[2] > bluescalarlow.val[2]&& rightmean[2] < bluescalarhigh.val[2]){
                ...
                if (current != 1){
                    if (centermean[0] > bluescalarlow.val[0] && centermean[0] < bluescalarhigh.val[0]){
                        if (centermean[1] > bluescalarlow.val[1] && centermean[1] < bluescalarhigh.val[1]){
                            if (centermean[2] > bluescalarlow.val[2] && centermean[2] < bluescalarhigh.val[2]){
